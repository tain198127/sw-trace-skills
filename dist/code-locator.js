...
function extractClassMethod(span) {
...
function findJavaFile(baseDir, className) {
...
function findMethodLine(content, methodName) {
...
function findEndpointInFile(content, endpoint) {
    const lines = content.split("\n");
    const patterns = [
...
function grepSearch(baseDir, keyword, maxResults = 5) {
    const results = [];
...
function getSnippet(content, line, contextLines = 3) {
    const lines = content.split("\n");
...
export function locateCode(span, codebases) {
    const locations = [];
...
export function locateAllSpans(spans, codebases) {
    const results = new Map();
    const key = (s) => `${s.segmentId}:${s.spanId}`;
    for (const span of spans) {
        if (span.component === "HttpClient" && !span.endpointName)
            continue;
