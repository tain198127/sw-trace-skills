...
function formatLabel(node: SpanNode): string {
  if (node.endpointName) return `${node.serviceCode}: ${node.endpointName}`;
  if (node.component && node.peer) return `${node.component}: ${node.peer}`;
  if (node.component) return `${node.component}: ${node.spanType}`;
  return `${node.serviceCode}: ${node.spanType}`;
}

function formatTags(node: SpanNode): string {
  if (node.tags.length === 0) return "";
  return node.tags.map((t) => `${t.key}=${t.value}`).join("; ");
}

export function writeTraceCsv(root: SpanNode, outputPath: string): string {
  ...
  for (const { node, depth: d } of rows) {
    ...
    const duration =
      node.endTime > node.startTime ? node.endTime - node.startTime : 0;
    const metaCols = [
