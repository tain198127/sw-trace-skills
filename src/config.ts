...
export function getCurrentRepo(): string | null {
  ...
  try {
    ...
    if (!name) return null;
    if (existsSync(join(REPOS_DIR, name, "config.yaml"))) return name;
    return null;
  } catch {
    return null;
  }
...
export function setCurrentRepo(name: string): void {
  ensureDirs();
  writeFileSync(CURRENT_FILE, name, "utf-8");
}

export function repoConfigPath(repoName: string): string {
...
export function initRepo(repoName: string, config?: Partial<SwConfig>): string {
  ensureDirs();
  const repoDir = join(REPOS_DIR, repoName);
  mkdirSync(repoDir, { recursive: true });

  const fullConfig: SwConfig = {
...
export function loadConfig(repoName: string): SwConfig {
...
export function saveConfig(repoName: string, config: SwConfig): string {
  ensureDirs();
  const repoDir = join(REPOS_DIR, repoName);
  mkdirSync(repoDir, { recursive: true });
...
export function resolvePromptOverride(config: SwConfig): string | null {
